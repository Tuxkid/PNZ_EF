...
### Separate into 4 dataframes
  ...
  ght.df <- df.sort(ght.df, c("Efppm", "Rep", "Duration", "Lifestage"))
  ght.df$Species <- "GHT"

  sjs.df <- aug.df[aug.df$Species == "San Jose Scale",]
  sjs.df <- df.sort(sjs.df, c("Efppm", "Rep", "Duration", "Lifestage"))
...
browser()
  ...
  cutx <- NULL
  leg.brief <- with(use.df, unique(paste(Species, Lifestage, ppaste(Duration, "h"),
                                         Rep, sep= "|")))
  maint <- "Mortality of various pests ON fruit in ethyl formate after one hour"
  xlabels <- c(0, 0)
  xaxtitle <- "Dose (%)"
  with(use.df,
