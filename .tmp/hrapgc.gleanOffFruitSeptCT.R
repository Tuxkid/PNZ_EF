gleanOffFruitSeptCT <- function(choice = 1)
{
### Purpose:- Uses CT product instead of concentration
### ----------------------------------------------------------------------
### Modified from:- gleanOffFruitSept
### ----------------------------------------------------------------------
### Arguments:- 
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 21 Sep 2015, 11:48
### ----------------------------------------------------------------------
### Revisions:- 
...
##  use.df <- within(use.df, DAT <- paste0(Assessed, "d"))
  leg.brief <- with(use.df, unique(paste(SLS, Temp,
                              Hours, Rep, sep= "|")))
  maint <- "Mortality of PNZ targets off fruit in ethyl formate after various durations (CT)"
  xlabels <- c(0, 0)
  xaxtitle <- "CT product (%-h)"
  with(use.df,
