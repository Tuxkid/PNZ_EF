gleanWithFruitSeptAll <- function(choice = 1)
{
### Purpose:- All species, not just LBAM
### ----------------------------------------------------------------------
### Modified from:- gleanWithFruitSeptLBAM
### ----------------------------------------------------------------------
### Arguments:- 
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 30 Sep 2015, 16:06
### ----------------------------------------------------------------------
### Revisions:- 
...
### Then a normal glean-type function
  ...
  leg.brief <- with(xx, unique(paste(SLS, casefold(substring(Fruit, 1, 1), upper = TRUE),
                                     Temp, Hours, Rep, sep= "|")))
  maint <- "Mortality of PNZ pests with fruit in ethyl formate after various durations"
  xlabels <- c(0, 0)
  xaxtitle <- "Dose (%)"
  with(xx,
