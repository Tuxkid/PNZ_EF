plot.presentationA <- function(titl = FALSE, span = .8 )
{
### Purpose:- PNGs for PowerPoint slides Including Moribundd=Dead
### ----------------------------------------------------------------------
### Modified from:- plot.presentation2
### ----------------------------------------------------------------------
### Arguments:- 
### ----------------------------------------------------------------------
...
### ALCM_ml
  ...
  pchs <- PCHS
  span <- span
  alcm.lo <- with(alcmML.df, loess(AngMort ~ Efppm, degree = 1, span = span))
  alcm.lo.pred <- predict(alcm.lo, unique(alcmML.df$Efppm), se = TRUE)
  ...
  alcm.lo.pred$fit[alcm.lo.pred$fit > 90] <- 90
  spline.i <- spline(unique(alcmML.df$Efppm), alcm.lo.pred$fit, n = 200)
  spline.i$y[spline.i$y > 90] <-  90
   spline.i$y <- monotone(spline.i$y)# take out some more bends sometimes
   
### plotting begins
  bitmap("ALMC_PP_ml.png", type = "png16m", width = 155, height = 143,
         res = 900, units = "mm", pointsize = 11)
  par(lwd = 2, font = 2)
  with(alcmML.df, blank.plot(Efppm, AngMort, ylim = c(0, 90)))
  ...
  with(alcmML.df, points(Efppm, AngMort, pch = pchs[1], #lty = df.i$lt[1],
...
### ALCM_md
  ...
  alcm.lo <- with(alcmMD.df, loess(AngMort ~ Efppm, degree = 1, span = span))
  alcm.lo.pred <- predict(alcm.lo, unique(alcmMD.df$Efppm), se = TRUE)
  alcm.lo.pred$fit[alcm.lo.pred$fit > 90] <- 90
  sems <- NULL # silly here but works when there's more than one line
  sems <- c(sems, rms(alcm.lo.pred$se.fit)) # make vector of  sems
  alcm.lo.pred$fit[alcm.lo.pred$fit > 90] <- 90
  spline.i <- spline(unique(alcmMD.df$Efppm), alcm.lo.pred$fit, n = 200)
  spline.i$y[spline.i$y > 90] <-  90
   spline.i$y <- monotone(spline.i$y)# take out some more bends sometimes
   
### plotting begins
  bitmap("ALMC_PP_md.png", type = "png16m", width = 155, height = 143,
         res = 900, units = "mm", pointsize = 11)
  par(lwd = 2, font = 2)
  with(alcmMD.df, blank.plot(Efppm, AngMort, ylim = c(0, 90)))
  axis(1, las = 1, mgp = c(1.8, .8, 0), cex.axis = .8, lwd = 2, font.axis = 2)
  axis(2, las = 1, at = ytix.at, labels = ytix, font.axis = 2, 
       mgp = c(1.8, .8, 0), cex.axis = ax.cex, lwd = 2)
  box()
  ...
  mtext("Mortality (%)", side = 2, line = 1.9, cex = lab.cex, font = 2)
  with(spline.i, lines(x, y, lty = 1, lwd = 3.5, col = colours[1]))
  with(alcmMD.df, points(Efppm, AngMort, pch = pchs[1], #lty = df.i$lt[1],
                       col = colours[1], cex = cex.pt))
  if(titl)
    title("ALCM_md")
  bar.legs(.6, .5, ebv = sems, ltys = rep(1, 3)[1], bar.lwd = 2,
           labs = "diapausing", pchs = pchs[1], font = 2,
           point.cex = cex.pt, leg.cex = leg.cex, cols = colours[1],
           eq.tex.gap = FALSE, line.lwd = 3.5, line.leng = .12)
  dev.off() # ploting of this one ends
