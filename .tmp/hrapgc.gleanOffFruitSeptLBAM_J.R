gleanOffFruitSeptLBAM_J <- function(choice = 1)
{
### Purpose:- Reps joined to give overall fit
### ----------------------------------------------------------------------
### Modified from:- gleanOffFruitSeptLBAM
### ----------------------------------------------------------------------
### Arguments:- 
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 24 Sep 2015, 14:12
### ----------------------------------------------------------------------
### Revisions:- 
  ...
  xx <- xx[!xx$Row %in% ignore.rows, ] %>%
    arrange(SLS, Temperature, Duration, Rep, Efpc, HC) %>%
      select(SLS, Temperature, Duration, Efpc, dead, Total)
...
### Then a normal glean function
  ...
  leg.brief <- with(use.df, unique(paste(SLS, Temp,
                                         Hours, sep= "|")))
  maint <- "Mortality of LBAM off fruit in ethyl formate after various durations (Joined)"
  xlabels <- c(0, 0)
  xaxtitle <- "Dose (%)"
  with(use.df,
