...
{
### Purpose:- September 2015 lot
### ----------------------------------------------------------------------
### Modified from:- group.responsesT1 -- need to change 'want' probably
### ----------------------------------------------------------------------
### Arguments:- 
### ----------------------------------------------------------------------
...
### Revisions:- 25/11/15 added 'change ME to Egg'
  ...
  xx$SLS <- as.character(xx$SLS)
 ## xx$SLS <- gsub("5", "5*", xx$SLS)
...
##    with(dfs, print(table(Duration, Temperature)))
    ...
    for(D in temps){ # not customised for Tem
      ...
      if(post){
        ##    browser()
        filename <- ppaste("Lines_for", sl, "_", D, "CS.pdf")
