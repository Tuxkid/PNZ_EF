group.responsesCT <- function(xx = june15Off.df, xtitle = "CT (%-hr)", main = "", yfun = "cloglog", 
           plimits = c(0.04, 0.9995), xlab.line = 2, x.mgp = 0.1,
           x.range = c(0, 6),
           rha = "Temperature", ylab.line = 1.6, lab.cex = .9, ax.cex = 0.85,
           main.cex = 1, post = TRUE, ...)
{
### Purpose:- CT product instead of dose
### ----------------------------------------------------------------------
### Modified from:- group.responses2
### ----------------------------------------------------------------------
### Arguments:- 
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 12 Jun 2015, 11:58
### ----------------------------------------------------------------------
### Revisions:- 


  if(post){
    pdf(file = ppaste("Multimort_by", rha, "CT.pdf"),
  ...
  ab <- ab.june15Off[[2]] # could be made adjustable
  data.fun <- ab$datafun
...
### Aim is to draw lines for each of tempsD available for this D
      ...
      names(interceptsD) <- names(slopesD) <-  names(concsD) <- durationsDh
...
### Draw each mortality line and a legend
      for(i in names(concsD))
        try(clipline(c(0, concsD[i]), c(-3, 2), interceptsD[i], slopesD[i],
