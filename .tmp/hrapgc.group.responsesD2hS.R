...
{
### Purpose:- Lots of species 2hr responses at 5 or 15 deg (September)
### ----------------------------------------------------------------------
### Modified from:- group.responsesD2h
### ----------------------------------------------------------------------
### Arguments:-  store: storage temp
###             hours: treatment duration
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 14 Oct 2015, 11:44
### ----------------------------------------------------------------------
### Revisions:- 
  ...
  sls.all <- sort(unique(as.character(yy$SLS)))## all SLS for this duratioon
...
##
  ...
  if(post){
     ##    browser()
    filename <- paste0("Lines_for", temps, "C_", hours, "hS.pdf")
    pdf(file = filename,
...
### Draw each mortality line and a legend
      ...
      bar.legs(.72, .44, labs =leg.labs, cols = colours[names(concsD)][draw.leg], pchs = NULL,
               ...
               ltys = ltys[draw.leg], line.leng = .1, line.lwd = 3)
    }  
}
