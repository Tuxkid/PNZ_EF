group.responsesCT1 <-
  function(xx = june15Off.df, xtitle = "CT (%-hr)", main = "", yfun = "cloglog", 
           plimits = c(0.04, 0.9995), xlab.line = 2, x.mgp = 0.1,
           x.range = c(0, 6), want = 1, ab.list = ab.june15Off, 
...
### Purpose:- 
### ----------------------------------------------------------------------
### Modified from:- group.responsesT1 & group.responsesCT
### ----------------------------------------------------------------------
### Arguments:- 
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 15 Jun 2015, 13:51
### ----------------------------------------------------------------------
### Revisions:- 16th July 2015 add in ab.list arg.
...
##    with(dfs, print(table(Duration, Temperature)))
    ...
    for(D in temps){ # not customised for Tem
      ...
      if(post){
        ##    browser()
        filename <- ppaste("Lines_for", sl, "_", D, "CT.pdf")
