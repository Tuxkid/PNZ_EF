fix.septWith2 <- function(xx = sept15With.df, dose.resp = TRUE)
{
### Purpose:- Gets codling moth data separate from others
### ----------------------------------------------------------------------
### Modified from:- fix.septWith
### ----------------------------------------------------------------------
### Arguments:- dose.resp: Dose response only?
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 20 Oct 2015, 14:31
### ----------------------------------------------------------------------
### Revisions:- 
  ...
  xx <- within(xx, Efnom[as.character(Efnom) == "CO2 Control"] <- 0)
  ...
  ## Tidy up SLS levels
  xx <- within(xx, SLS <- as.character(SLS))
  xx <- within(xx, SLS[SLS == "CM5"] <- "CM 5")
  xx <- within(xx, SLS[SLS == "Cm5"] <- "CM 5")
  xx <- within(xx, SLS[SLS == "Cm 5"] <- "CM 5")
  xx <- within(xx, SLS[SLS == "CM Eggs"] <- "CM Egg")
  xx <- within(xx, SLS[SLS == "CM eggs"] <- "CM Egg")
  xx <- xx[grep("CM ", xx$SLS),]
  xx <- xx[grep("Dose", xx$Fruit),] # i.e. a dose response c.f. semi-commercial
  xx <- within(xx, SLS <- factor(SLS))
  ...
  xx <- within(xx, IsScale <- SLS%in%is.scale) # redundant in this incarnation which is only CM
...
### Define what is dead
  ...
  xxx <- xx %>%
    arrange(Date, SLS, Fruit, Temperature, Duration, Placement, Rep, Efpc) %>%
      select(Date, SLS, Fruit, Temperature, Duration, Placement, Rep, Efnom, Efpc, HC, dead, Total, Row)
  idcols <- names(xxx %>%
                  select(Date, SLS, Fruit, Temperature, Duration, Placement, Rep))
  respcols <- names(xxx %>%
                    select(dead, Total, Row))

  xxx <- within(xxx, Ndx <- paste(Date, Placement, Rep, sep = "|"))
...
##     browser()
    ...
    if(nrow(treat.i) < 1){
      ...
      if(nrow(hand.i) == 0){
        hand.i <- treat.i[1, ]
        hand.i[, respcols] <- NA ##!!!! suspect
        hand.i[, c("Efnom", "Efpc", "HC")] <- c(0, 0, 1)
      }
      cont.i <- rbind(hand.i) # get controls back together
      if(nrow(co2.i) == 0){
        co2.i <- treat.i[1, ]
        co2.i[, respcols] <- NA
        co2.i[, c("Efnom", "Efpc", "HC")] <- c(0, 0, 0)
      }
...
##         Ndx.k <- missing.k$Ndx
##         Ndx.kFix <- gsub("\\|2\\|", "|3|", Ndx.k) # won't work here since there's no Duration 3
...
##   }
  ...
  use.df <- ditch("Ndx", rbind(fixed.cont.df, cont.notmissing.df, xx.treat)) %>%
                  arrange(Date, SLS, Fruit, Temperature, Duration, Placement, Rep, Efnom, Efpc, HC) %>%
                    filter(!is.na(Total))
  use.df <- within(use.df,
                   Ndx <- paste(Date, Rep, sep = "|"))
## needs summation of samples
  use.sum <- use.df %>%
    group_by(Placement, Rep, factor(Efnom), factor(Efpc), HC) %>%
      summarise_each(funs(sum), dead, Total)
##   names(use.sum) <- gsub(("factor\\("), "", names(use.sum))
##   names(use.sum) <- gsub(("\\)"), "", names(use.sum))
##browser()
  use.sum <- as.data.frame(use.sum) # otherwise the next line screws up
  names(use.sum) <- gsub(("factor\\(|\\)"), '', names(use.sum))
  
##   if(FALSE){
    
    WriteXLS(c("fixed.cont.df", "contrmissing.dfB4", "contronly.df", "use.df", "use.sum"),
             "MissingCM.xls",
             c("AllControls", "NoControls", "ControlsOnly", "ReadyToUse", "ForMortalityPlots"),
